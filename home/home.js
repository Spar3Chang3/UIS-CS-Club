...
function runIntroObserver(observerWrapper) {
    ...
    if (isMobile.matches) {
        ...
        children = observerWrapper.querySelectorAll(".intro-text, .events, .event-calendar-h2, .calendar, .links");
    }
...
function initObserverAnimations() {
    const children = document.querySelectorAll(".intro-text, .events, .event-calendar-h2, .calendar, .links");
    for (const child of children) {
        child.style = "height: 0; transform: translateY(100%); opacity: 0; visibility: hidden;";
    }
}
